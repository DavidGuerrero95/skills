...
2. Read the applicable policies, rules, and the active stack profile:
   - `policies/01-engineering-baseline.md`
   - `policies/02-clean-architecture.md`
   - `policies/03-async-and-messaging.md` (when relevant)
   - `stacks/<stack>.md`
...
5. Run a code-smell pass (delegate to `agents/code-smell-auditor`).
6. Update documentation when behavior or contracts changed
   (delegate to `agents/technical-writer`).
7. If the change is cross-module, run smoke / E2E
   (delegate to `agents/e2e-test-engineer`).

## Recommended delegates

- `implementation-engineer` (lead)
- `unit-test-engineer`
- `code-smell-auditor`
- `database-engineer` when schema / migrations change
