...
2. **Async correctness** — `policies/03-async-and-messaging.md`.
...
6. **Tests** — coverage of touched paths, regression for fixes,
   determinism, focused assertions.
7. **Docs** — README, ADRs, runbooks, contract docs, env vars.
8. **Domain safety** (when applicable) —
   `policies/06-domain-guardrails.md`, `skills/domain-safety-review`.

## Recommended delegates

- `code-reviewer` (lead, independent diff review)
- `software-architect` (when boundary concerns appear)
- `code-smell-auditor` (when readability concerns appear)
- `database-engineer` (when schema / query concerns appear)
