...
Run a bounded, behavior-preserving refactor on a single module. Use when
...
- A unit with mixed responsibilities that needs to split.
- Long functions that obscure intent.
...
4. **Apply each step** as its own commit. Run the module's tests between
   steps.
5. **Update neighbors** only when truly required.
6. **Document the outcome** if the change is significant
   (`docs/architecture/`, an ADR if the placement choice was
   deliberate).

## Recommended delegates

- `software-architect` (placement decisions)
